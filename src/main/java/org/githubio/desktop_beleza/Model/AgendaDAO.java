...
public class AgendaDAO {
    ...
    // --- MÉTODO ORIGINAL (todos os agendamentos) ---
    ...
    // --- MÉTODO NOVO: filtra por semana ---
    public List<Agenda> listarAgendamentosPorSemana(LocalDate inicioSemana, LocalDate fimSemana) {
        List<Agenda> lista = new ArrayList<>();
        String sql = "SELECT a.id_agenda, a.data_agenda, a.horario_agenda, " +
                "s.nome_servico, m.nome_modelo, sa.status_agenda " +
                "FROM tb_agenda a " +
        ...
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, inicioSemana.toString());  // "2026-04-13"
            stmt.setString(2, fimSemana.toString());     // "2026-04-19"
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                lista.add(new Agenda(
                        rs.getInt("id_agenda"),
